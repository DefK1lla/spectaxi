...
- `published -> cancelled` — отмена клиентом без причины; **системная отмена** «исполнитель не найден» (раздел 4.2); отмена админом
...
- срочный — через **24 часа** после `published_at` (поле обновляется при каждом входе в `published`);
